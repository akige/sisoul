...
| **Stablecoin micropay** (default for `emergency-only` & cross-strangers) | Strangers; emergencies; "I need 100k tokens right now" | Direct USDT-TRC20 payment to lender's wallet | Borrower pays lender ~$0.01 / 1000 tokens (configurable per lender). **Currently 0% protocol fee — no rake, no escrow.** A low protocol fee (2-3%) funding the dev team may be introduced later via the L2 settlement contract: transparently disclosed, opt-out-able, never a token. | ✅ — USDT is issued by Tether, not by sisoul; the borrower pays the lender directly. We never custody the money. |
